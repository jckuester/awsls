...
func ListDxHostedTransitVirtualInterface(client *Client) {
	req := client.directconnectconn.DescribeVirtualInterfacesRequest(&directconnect.DescribeVirtualInterfacesInput{})

	resp, err := req.Send(context.Background())
	if err != nil {
		log.Printf("aws_dx_hosted_transit_virtual_interface: %s", err)
	} else {
		if len(resp.VirtualInterfaces) > 0 {
			fmt.Println("")
			fmt.Println("aws_dx_hosted_transit_virtual_interface:")
