...
func ListResources(client *Client) error {
	ListAccessanalyzerAnalyzer(client)
	ListAcmCertificate(client)
	ListAlbListener(client)
	ListAlbListenerRule(client)
	ListAlbTargetGroup(client)
	ListAmi(client)
	ListApiGatewayApiKey(client)
	ListApiGatewayClientCertificate(client)
	ListApiGatewayDomainName(client)
	ListApiGatewayRestApi(client)
	ListApiGatewayUsagePlan(client)
	ListApiGatewayVpcLink(client)
	ListApigatewayv2Api(client)
	ListAppmeshMesh(client)
	ListAppsyncGraphqlApi(client)
	ListAthenaWorkgroup(client)
	ListAutoscalingGroup(client)
	ListBackupPlan(client)
	ListBackupVault(client)
	ListBatchComputeEnvironment(client)
	ListBatchJobDefinition(client)
	ListBatchJobQueue(client)
	ListCloudformationStack(client)
	ListCloudformationStackSet(client)
	ListCloudhsmV2Cluster(client)
	ListCloudwatchDashboard(client)
	ListCloudwatchEventRule(client)
	ListCloudwatchLogDestination(client)
	ListCloudwatchLogGroup(client)
	ListCloudwatchLogResourcePolicy(client)
	ListCodebuildSourceCredential(client)
	ListCodecommitRepository(client)
	ListCodepipelineWebhook(client)
	ListCodestarnotificationsNotificationRule(client)
	ListConfigConfigRule(client)
	ListConfigConfigurationRecorder(client)
	ListConfigDeliveryChannel(client)
	ListCurReportDefinition(client)
	ListCustomerGateway(client)
	ListDatasyncAgent(client)
	ListDatasyncTask(client)
	ListDaxParameterGroup(client)
	ListDaxSubnetGroup(client)
	ListDbEventSubscription(client)
	ListDbInstance(client)
	ListDbParameterGroup(client)
	ListDbSecurityGroup(client)
	ListDbSubnetGroup(client)
	ListDefaultRouteTable(client)
	ListDefaultSecurityGroup(client)
	ListDefaultVpc(client)
	ListDevicefarmProject(client)
	ListDlmLifecyclePolicy(client)
	ListDmsCertificate(client)
	ListDmsEndpoint(client)
	ListDmsReplicationSubnetGroup(client)
	ListDmsReplicationTask(client)
	ListDxConnection(client)
	ListDxHostedPrivateVirtualInterface(client)
	ListDxHostedPublicVirtualInterface(client)
	ListDxHostedTransitVirtualInterface(client)
	ListDxLag(client)
	ListDxPrivateVirtualInterface(client)
	ListDxPublicVirtualInterface(client)
	ListDxTransitVirtualInterface(client)
	ListDynamodbGlobalTable(client)
	ListEbsSnapshot(client)
	ListEbsVolume(client)
	ListEc2CapacityReservation(client)
	ListEc2ClientVpnEndpoint(client)
	ListEc2Fleet(client)
	ListEc2TrafficMirrorFilter(client)
	ListEc2TrafficMirrorSession(client)
	ListEc2TrafficMirrorTarget(client)
	ListEc2TransitGateway(client)
	ListEc2TransitGatewayRouteTable(client)
	ListEc2TransitGatewayVpcAttachment(client)
	ListEcrRepository(client)
	ListEcsCluster(client)
	ListEfsFileSystem(client)
	ListEfsMountTarget(client)
	ListEgressOnlyInternetGateway(client)
	ListEip(client)
	ListElasticBeanstalkApplication(client)
	ListElasticBeanstalkApplicationVersion(client)
	ListElasticBeanstalkEnvironment(client)
	ListElasticacheReplicationGroup(client)
	ListElastictranscoderPipeline(client)
	ListElastictranscoderPreset(client)
	ListElb(client)
	ListEmrSecurityConfiguration(client)
	ListFsxLustreFileSystem(client)
	ListFsxWindowsFileSystem(client)
	ListGameliftAlias(client)
	ListGameliftBuild(client)
	ListGameliftGameSessionQueue(client)
	ListGlobalacceleratorAccelerator(client)
	ListGlueCrawler(client)
	ListGlueJob(client)
	ListGlueSecurityConfiguration(client)
	ListGlueTrigger(client)
	ListIamAccessKey(client)
	ListIamGroup(client)
	ListIamInstanceProfile(client)
	ListIamPolicy(client)
	ListIamRole(client)
	ListIamServerCertificate(client)
	ListIamServiceLinkedRole(client)
	ListIamUser(client)
	ListInternetGateway(client)
	ListIotCertificate(client)
	ListIotPolicy(client)
	ListIotThing(client)
	ListIotThingType(client)
	ListIotTopicRule(client)
	ListKeyPair(client)
	ListKinesisAnalyticsApplication(client)
	ListKmsAlias(client)
	ListKmsExternalKey(client)
	ListKmsKey(client)
	ListLambdaEventSourceMapping(client)
	ListLambdaFunction(client)
	ListLaunchConfiguration(client)
	ListLaunchTemplate(client)
	ListLbListener(client)
	ListLbListenerRule(client)
	ListLbTargetGroup(client)
	ListLicensemanagerLicenseConfiguration(client)
	ListLightsailDomain(client)
	ListLightsailInstance(client)
	ListLightsailKeyPair(client)
	ListLightsailStaticIp(client)
	ListMediaConvertQueue(client)
	ListMediaPackageChannel(client)
	ListMediaStoreContainer(client)
	ListMqBroker(client)
	ListMqConfiguration(client)
	ListMskCluster(client)
	ListMskConfiguration(client)
	ListNatGateway(client)
	ListNeptuneEventSubscription(client)
	ListNetworkAcl(client)
	ListNetworkInterface(client)
	ListOpsworksInstance(client)
	ListOpsworksStack(client)
	ListOpsworksUserProfile(client)
	ListPlacementGroup(client)
	ListQldbLedger(client)
	ListRdsGlobalCluster(client)
	ListRedshiftCluster(client)
	ListRedshiftEventSubscription(client)
	ListRedshiftSnapshotCopyGrant(client)
	ListRedshiftSnapshotSchedule(client)
	ListRoute53HealthCheck(client)
	ListRoute53ResolverEndpoint(client)
	ListRoute53ResolverRule(client)
	ListRoute53ResolverRuleAssociation(client)
	ListRoute53Zone(client)
	ListRouteTable(client)
	ListS3Bucket(client)
	ListSagemakerEndpoint(client)
	ListSagemakerModel(client)
	ListSecretsmanagerSecret(client)
	ListSecurityGroup(client)
	ListServiceDiscoveryService(client)
	ListServicecatalogPortfolio(client)
	ListSesActiveReceiptRuleSet(client)
	ListSesConfigurationSet(client)
	ListSesReceiptFilter(client)
	ListSesReceiptRuleSet(client)
	ListSesTemplate(client)
	ListSfnActivity(client)
	ListSfnStateMachine(client)
	ListSnsPlatformApplication(client)
	ListSnsTopic(client)
	ListSnsTopicSubscription(client)
	ListSpotFleetRequest(client)
	ListSpotInstanceRequest(client)
	ListSsmActivation(client)
	ListSsmAssociation(client)
	ListSsmDocument(client)
	ListSsmMaintenanceWindow(client)
	ListSsmPatchBaseline(client)
	ListSsmPatchGroup(client)
	ListStoragegatewayGateway(client)
	ListSubnet(client)
	ListTransferServer(client)
	ListVpc(client)
	ListVpcEndpoint(client)
	ListVpcEndpointConnectionNotification(client)
	ListVpcEndpointService(client)
	ListVpcPeeringConnection(client)
	ListVpnGateway(client)
	ListWafByteMatchSet(client)
	ListWafGeoMatchSet(client)
	ListWafIpset(client)
	ListWafRateBasedRule(client)
	ListWafRegexMatchSet(client)
	ListWafRegexPatternSet(client)
	ListWafRule(client)
	ListWafRuleGroup(client)
	ListWafSizeConstraintSet(client)
	ListWafSqlInjectionMatchSet(client)
	ListWafWebAcl(client)
	ListWafXssMatchSet(client)
	ListWafregionalByteMatchSet(client)
	ListWafregionalGeoMatchSet(client)
	ListWafregionalIpset(client)
	ListWafregionalRateBasedRule(client)
	ListWafregionalRegexMatchSet(client)
	ListWafregionalRegexPatternSet(client)
	ListWafregionalRule(client)
	ListWafregionalRuleGroup(client)
	ListWafregionalSizeConstraintSet(client)
	ListWafregionalSqlInjectionMatchSet(client)
	ListWafregionalWebAcl(client)
	ListWafregionalXssMatchSet(client)
	ListWorklinkFleet(client)
	ListWorkspacesIpGroup(client)
